...
- **IF improvement_score ≥ 50%:**
  - **REPLAN PATH:**
    ...
    - Recommend: "Invoke ln-300-task-coordinator with mode=REPLAN"
...
- Test Strategy section must exist but remain empty (testing handled by ln-510 later)
...
- **Display tabular output** (Unicode box-drawing) to terminal:
  - STORY VALIDATION REPORT header with Story ID + ln-310 version
...
|7 Test Strategy|Section exists but remains empty now|Ensure Test Strategy section present; leave empty with note that testing is planned later by ln-510; do not add coverage details|Mention "testing handled later; not evaluated in this phase"|
...
|9 Story Size & Granularity|3-8 tasks; 3-5h each|If <3 tasks invoke ln-300-task-coordinator; if >8 add TODO to split; flag tasks <3h or >8h with TODO; reload metadata and update kanban_board.md|Comment creation source|
...
- **Templates:** `../ln-220-story-coordinator/references/story_template_universal.md`, `../ln-301-task-creator/references/task_template_implementation.md`
...
- **Testing methodology** (for later phases): `../ln-510-test-planner/references/risk_based_testing_guide.md`
