...
Execute approved Story Finalizer test tasks (Todo → In Progress → To Review) created by ln-350-story-test-planner with comprehensive 11-section structure.

## When to Use This Skill

This skill should be used when executing Story Finalizer test task (status = Todo, created by ln-350-story-test-planner) with 11 sections:
...
> For implementation tasks (without test logic), use **ln-331-task-executor** skill instead.
...
- > [!WARNING]
- > All existing tests MUST pass before proceeding to Step 2

**Step 2: Implement New Tests (Sections 3-5, E2E-first Risk-Based Testing, Priority ≥15)**

> [!NOTE]

> Risk assessment completed by ln-350-story-test-planner. Task contains exact scenarios with Priority scores.
...
**Reference:** See `ln-350-story-test-planner/references/risk_based_testing_guide.md` for complete Risk-Based Testing methodology, test limits (2-5/3-8/5-15, total 10-28 max), and test selection criteria.
...
3. **Follow task description:** All 11 sections contain specific instructions from ln-350-story-test-planner analysis
