# ln-321 Auto-Trigger Matrix

Mapping Story patterns to documentation types for automatic ln-321 invocation during ln-320 validation.

---

## Purpose

This matrix defines WHEN to invoke ln-321-docs-creator during Story validation and WHAT type of document to create.

**Usage in ln-320 SKILL.md:**
```markdown
### Phase 2: Critical Solution Review (Step 6 - Auto-Trigger ln-321)
1. Load ln321_auto_trigger_matrix.md
2. Scan Story title + Technical Notes for trigger keywords
3. IF keywords match → Invoke ln-321 with doc_type + topic from matrix
...
IF Story matches "Rate Limiting" pattern:
  → Invoke: Skill(skill="ln-321-docs-creator", args="doc_type=guide topic='API Rate Limiting Pattern'")
...
IF Story matches "OAuth/OIDC" pattern:
  ...
  → Invoke: Skill(skill="ln-321-docs-creator", args="doc_type=manual topic='oauth2-proxy v7.6.0'")
  → Output: docs/manuals/oauth2-proxy-v7.md
  → Invoke: Skill(skill="ln-321-docs-creator", args="doc_type=adr topic='Authentication Strategy'")
...
FOR ALL Stories:
  IF "Error Handling" subsection missing:
    → Invoke: Skill(skill="ln-321-docs-creator", args="doc_type=guide topic='Error Response Patterns (RFC 7807)'")
...
FOR ALL Stories:
  IF "Logging" subsection missing:
    → Invoke: Skill(skill="ln-321-docs-creator", args="doc_type=guide topic='Structured Logging Strategy'")
...
**ln-321 Invocations:**
1. `Skill(skill="ln-321-docs-creator", args="doc_type=manual topic='oauth2-proxy v7.6.0'")`
   - Creates: `docs/manuals/oauth2-proxy-v7.md`
2. `Skill(skill="ln-321-docs-creator", args="doc_type=adr topic='Authentication Strategy'")`
...
**ln-321 Invocations:**
1. `Skill(skill="ln-321-docs-creator", args="doc_type=guide topic='RESTful API Patterns'")`
   - Creates: `docs/guides/05-rest-api-patterns.md`
2. `Skill(skill="ln-321-docs-creator", args="doc_type=guide topic='API Rate Limiting Pattern'")`
...
**ln-321 Invocations:**
1. `Skill(skill="ln-321-docs-creator", args="doc_type=manual topic='Prisma v5.8.1'")`
   - Creates: `docs/manuals/prisma-5.md`
2. `Skill(skill="ln-321-docs-creator", args="doc_type=manual topic='Redis v7.2'")`
