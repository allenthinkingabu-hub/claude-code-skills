...
Do NOT use if:
- Manual testing NOT completed → Wait for ln-500-story-quality-gate Pass 1
- Manual test results NOT in Linear comment → ln-500-story-quality-gate must document first
...
3. Load manual test results comment (format: ln-503-manual-tester Format v1.0)
   ...
   - If not found → ERROR: Run ln-500-story-quality-gate Pass 1 first
...
**Error Handling:** Missing comment → ERROR (run ln-500-story-quality-gate Pass 1 first), Missing format version → WARNING (try legacy parsing), Required section missing → ERROR (re-run ln-500-story-quality-gate), No PASSED AC → ERROR (fix implementation)
...
- **autoApprove: true** (default for ln-500-story-quality-gate) → proceed automatically with no user input
...
- [ ] REPLAN MODE (if count ≥ 1): Delegated to ln-302-task-replanner with taskType: "test"
