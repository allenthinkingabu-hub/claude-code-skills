...
- Part of Story Review (Phase 4 Step 3 in ln-330-story-executor)
- After regression tests pass (baseline stable)
- Before creating test task (manual testing informs test scenarios)

> [!NOTE]

> This is an atomic worker skill with single responsibility - it ONLY performs manual testing and documents results. It does NOT create fix tasks, change Story/Task statuses, or make workflow decisions.
...
> - **During work:** Mark `- [x]` checkboxes as steps complete (Docker rebuilt, AC tested, etc.)
> - **Before returning:**
>   - Mark `- [x]` final checkboxes with results (Verdict: PASS/FAIL, Temp script path)
>   - Record `| timestamp | ln-343 | Released | to ln-340 |` in Ownership Log
...
# Deleted by: ln-334-test-executor Step 6 (after E2E/Integration/Unit tests implemented)
...
- Deleted: ln-334-test-executor Step 6 (after automated tests implemented)
...
This skill does NOT:
- Create fix tasks in Linear (ln-330-story-executor creates tasks)
- Change Story or Task statuses (ln-330-story-executor manages statuses)
- Make workflow decisions (ln-330-story-executor decides next step)
- Run automated tests (that's ln-342-regression-checker or ln-334-test-executor)
- Create test task (that's ln-350-story-test-planner)

**Rationale:** Single Responsibility Principle - this skill ONLY performs manual testing and documents results. All workflow logic belongs to ln-330-story-executor orchestrator.
