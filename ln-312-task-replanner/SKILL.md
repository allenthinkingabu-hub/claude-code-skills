...
**1. ln-310-story-decomposer (REPLAN MODE):**
...
**3. ln-350-story-test-planner (Phase 6 - Manual Testing Changed):**
...
Read template from ln-311-task-creator/references/ based on `taskType` parameter:
...
Generate new doc (taskType determines template) → Validate (same rules as ln-311-task-creator Phase 2) → `update_issue(description)` → Add comment
...
**Validation rules (type-specific, same as ln-311-task-creator Phase 2):**
...
**Same validation logic as ln-311-task-creator Phase 2.**
...
**Location**: [ln-312-task-replanner/references/replan_algorithm.md](references/replan_algorithm.md)

**Ownership**: ln-312-task-replanner (worker-specific logic)

**Usage**: ln-312-task-replanner applies this algorithm in Phase 2 (Compare IDEAL vs Existing)

### Task Templates (ln-311-task-creator Ownership)

**This skill reads ALL 3 templates** from ln-311-task-creator/references/ directory:

**task_template_implementation.md:**
- **Purpose**: Implementation task template (7 sections, 146 lines)
- **Location**: [ln-311-task-creator/references/task_template_implementation.md](../ln-311-task-creator/references/task_template_implementation.md)
...
- **Location**: [ln-311-task-creator/references/refactoring_task_template.md](../ln-311-task-creator/references/refactoring_task_template.md)
...
- **Location**: [ln-311-task-creator/references/test_task_template.md](../ln-311-task-creator/references/test_task_template.md)
...
|--------------|---------|----------|------------------|
| ln-310-story-decomposer | Story AC/approach changed | implementation | idealPlan, guideLinks |
| ln-340-story-quality-gate | New code quality issues | refactoring | codeQualityIssues, refactoringPlan, affectedComponents |
| ln-350-story-test-planner | Manual test results changed | test | manualTestResults, testPlan, infra, docs, cleanup |
...
**DELETE Algorithm (OBSOLETE tasks):**
1. Load `docs/tasks/kanban_board.md`
2. For each Canceled task:
   - Find Epic group containing this task (search for `**Epic N:**`)
   - Find Story containing this task (search for `📖 [STORY_ID:`)
   - Remove task line: `    - [TASK_ID: ...]` (4-space indent)
   - If Story has no more tasks after removal:
     - If status is Backlog/Postponed: add note `_(tasks not created yet)_`
     - Otherwise: keep Story (with other tasks or as-is)
   - If Epic group has no more Stories: remove Epic header

**CREATE Algorithm (NEW tasks):**
1. Load `docs/tasks/kanban_board.md`
...
**Version:** 4.0.0 (BREAKING: Epic grouping algorithm for DELETE and CREATE operations)
