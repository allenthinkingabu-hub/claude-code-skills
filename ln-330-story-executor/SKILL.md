...
> This skill orchestrates full Story execution including explicit delegation to ln-340-story-quality-gate Pass 1 (when all implementation tasks Done) and Pass 2 (when test task Done).

## Workflow Overview

```
┌─────────────────────────────────────────────────────┐
│ ln-330-story-executor (TASK ORCHESTRATOR)                │
├─────────────────────────────────────────────────────┤
│ Phase 1: Discovery                                  │
│ Phase 2: Load Story & Task Metadata                 │
│                                                     │
│ Phase 3: Task Orchestration Loop ◄──────────┐       │
│   Priority 0: To Review → ln-332-task-reviewer   │       │
│   Priority 1: To Rework → ln-333-task-rework     │       │
...
│   → Invoke ln-340-story-quality-gate Pass 1          │       │
│     (Skill tool - Story quality worker)     │       │
│                                                     │
│   Pass 1 executes:                                  │
│     - ln-341-code-quality-checker (Fail Fast)            │
│     - ln-342-regression-checker (Fail Fast)              │
│     - ln-343-manual-tester (Fail Fast, Format v1.0)      │
│     - Verdict: Path A/B/C                           │
│                                                     │
│   Path A: Test task created (via ln-350-story-test-planner │
│           → ln-311-task-creator/replanner)               │
...
│                                                     │
│   Path C: No tasks created (test task already Done) │
│     → GOTO Action 4 (Invoke Pass 2)                 │
└─────────────────────────────────────────────────────┘
```

**Key Points:**
- **Orchestrator-Worker Pattern:** ln-330-story-executor orchestrates tasks, delegates Story quality to ln-340-story-quality-gate
- **Task-Level Orchestration:** Phase 3 manages task workflow (To Review → To Rework → Todo)
- **Story-Level Delegation:** Phase 4 delegates to ln-340-story-quality-gate Pass 1 for quality verification
- **Self-Healing Loop:** ln-340-story-quality-gate creates fix/refactor/test tasks → loops back to Phase 3
...
- ln-330-story-executor ONLY invokes executor skills with task ID parameter
- ln-330-story-executor does NOT update task status directly
...
- If tasks in To Review exist (from Linear metadata loaded in Phase 2):
  ...
  3. **Use Skill tool:** `Skill(skill: "ln-332-task-reviewer")` with task ID
  4. ln-332-task-reviewer loads full description and handles: To Review → Done OR To Review → To Rework
...
- If no To Review AND no To Rework tasks, but Todo tasks exist (from Linear metadata loaded in Phase 2):
  ...
  4. **Use Skill tool based on task type:**
     - **IF label "tests":** `Skill(skill: "ln-334-test-executor")` with task ID
       - ln-334-test-executor loads full test task description (11 sections from ln-350-story-test-planner task plan)
...
Exit when no tasks in these statuses → Phase 4 → Invoke ln-340-story-quality-gate Pass 1 for Story-level quality verification.
...
**Goal:** Delegate Story-level quality verification to ln-340-story-quality-gate Pass 1
...
**2. Invoke ln-340-story-quality-gate Pass 1:**
```
Skill(skill: "ln-340-story-quality-gate")
```
- Pass Story ID as parameter
- ln-340-story-quality-gate executes 6 phases:
...
- ln-340-story-quality-gate returns verdict + created task IDs (if any)

**3. Process ln-340-story-quality-gate response:**

**Path A: Test task created**
- ln-340-story-quality-gate created test task (taskType: "test") via ln-350-story-test-planner
- Test task status = Backlog
- **Auto-verify:** Invoke ln-320-story-validator with Story ID and test task ID (Backlog → Todo)
- Add comment to Story: "ln-340-story-quality-gate Pass 1 completed. Test task created: [TASK_ID]. Auto-verified and moved to Todo."
- **GOTO Phase 3:** Execute test task through orchestration loop
  - Priority 2: ln-334-test-executor invoked (test task has label "tests")
...
- ln-340-story-quality-gate found code quality issues
- Refactoring task created (taskType: "refactoring") via ln-311-task-creator
- Refactor task status = Backlog
- **Auto-verify:** Invoke ln-320-story-validator with Story ID and refactor task ID (Backlog → Todo)
- Add comment to Story: "ln-340-story-quality-gate Pass 1 completed. Refactoring task created: [TASK_ID]. Auto-verified and moved to Todo."
- **GOTO Phase 3:** Execute refactor task through orchestration loop
  - Priority 2: ln-331-task-executor invoked
...
- ln-340-story-quality-gate Pass 1 passed without creating tasks (rare case)
...
**4. Invoke ln-340-story-quality-gate Pass 2 (explicit delegation):**
...
3. Invoke ln-340-story-quality-gate Pass 2:
   ```
   Skill(skill: "ln-340-story-quality-gate")
   ```
4. ln-340-story-quality-gate Pass 2 executes:
...
**Output:**
- "ln-330-story-executor completed successfully"
- "Story [ID] status: Done"
- "All tasks completed and verified"

> [!NOTE]

> ln-330-story-executor now runs FULL pipeline automatically (Todo → In Progress → To Review → Done)

---

### Story Status Responsibility Matrix

**ln-330-story-executor responsibility**: Manages **2 of 4** Story status transitions (Task execution phase).

| Story Status Transition | Responsible Skill | When |
|-------------------------|-------------------|------|
| Backlog → Todo | ln-320-story-validator | After auto-fix and approval |
| **Todo → In Progress** | **ln-330-story-executor** | **First task execution starts (Phase 3 Priority 2)** |
| **In Progress → To Review** | **ln-330-story-executor** | **All tasks Done (Phase 4 transition)** |
| To Review → Done | ln-340-story-quality-gate Pass 2 | All tests verified, Priority ≥15 covered |

**ln-330-story-executor transitions (details):**
...
> ln-330-story-executor does NOT update To Review → Done (ln-340-story-quality-gate Pass 2 responsibility)

### L2→L2 Delegation: ln-340-story-quality-gate
...
|------------|------|-------------------|-----------|
| **ln-330-story-executor → ln-340-story-quality-gate Pass 1** | Phase 4: All implementation tasks Done | Task execution → Story quality validation | Delegate quality verification (Code Quality, Regression, Manual Testing) to specialized orchestrator |
| **ln-330-story-executor → ln-340-story-quality-gate Pass 2** | Phase 4 Action 4: Test task Done | Task execution → Final approval | Delegate final Story approval (test verification, Priority ≥15 coverage) to specialized orchestrator |

**Why L2→L2 (not L2→L3):**
- ln-340-story-quality-gate is orchestrator (invokes ln-350-story-test-planner, ln-311-task-creator, ln-343-manual-tester)
- Separates task execution (ln-330-story-executor) from quality validation (ln-340-story-quality-gate)
- Enables Self-Healing Pipeline: quality-coordinator creates fix/refactor/test tasks → loops back to ln-330-story-executor Phase 3

**Compliance with L2→L2 Rules:**
- ✅ Rule 1: Explicit Skill tool invocation (`Skill(skill: "ln-340-story-quality-gate")`)
...
**After Phase 4 (ln-340-story-quality-gate Pass 1) completes:**
- "ln-340-story-quality-gate Pass 1 completed: [VERDICT]"
...
- ln-340-story-quality-gate Pass 1 verdict
...
- User must resolve issue manually before re-running ln-330-story-executor
...
- Recommend: "Create tasks using ln-220-story-coordinator or ln-310-story-decomposer"

### Comparison with Other Skills

| Aspect | ln-330-story-executor | ln-331-task-executor | ln-332-task-reviewer | ln-340-story-quality-gate |
|--------|----------------|----------------|-------------|--------------|
...
| **Invokes** | ln-332-task-reviewer, ln-333-task-rework, ln-331-task-executor, ln-334-test-executor, ln-340-story-quality-gate (via Skill tool) | None | None | ln-350-story-test-planner, ln-311-task-creator, ln-312-task-replanner (via Skill tool) |
| **Status** | Todo → In Progress → To Review | Todo → In Progress → To Review | To Review → Done/Rework | Manual testing + Story Done |
| **When** | User wants to execute entire Story | ln-330-story-executor calls via Skill tool OR user directly | ln-330-story-executor calls via Skill tool OR user directly | ln-330-story-executor calls Pass 1 automatically |
...
- [ ] ln-340-story-quality-gate Pass 1 invoked automatically via Skill tool
- [ ] If ln-340-story-quality-gate creates tasks (test/refactor/fix):
  - [ ] Auto-verify via ln-320-story-validator (Backlog → Todo)
  - [ ] Loop back to Phase 3
- [ ] If test task Done: Story status updated In Progress → To Review
- [ ] ln-340-story-quality-gate Pass 2 invoked automatically via Skill tool
...
- ln-340-story-quality-gate Pass 1: Invoked automatically
- ln-340-story-quality-gate Pass 2: Invoked automatically
- Story lifecycle complete
- Progress report with task summary

---

## kanban_board.md Updates (Epic Grouping Algorithm)

**Story Movement Algorithm:**

When moving Story from source status (e.g., Todo) to target status (e.g., In Progress):

1. Load `docs/tasks/kanban_board.md`
2. Find Story in source section (e.g., `### Todo`)
3. Identify parent Epic group (`**Epic N: Epic Title**`)
4. Extract Epic section block:
   ```markdown
   **Epic N: Epic Title**

     📖 [STORY_ID: USXXX Title](link) ✅ APPROVED
       - [TASK_ID: EP#_01 Task](link)
       - [TASK_ID: EP#_02 Task](link)
   ```
5. Check if target section (e.g., `### In Progress`) has same Epic group:
   - **If exists**: Add Story block under existing Epic (preserve Epic header)
   - **If NOT exists**: Create new Epic group, then add Story block
6. Remove Epic section from source section
7. If source Epic group has other Stories: split Epic group (keep other Stories in source)

**Rules:**
- **Epic Context**: Always preserve Epic header when moving Stories
- **Epic Splitting**: If Epic has multiple Stories in same status, only move the target Story
- **Epic Merging**: If target status already has Epic group, merge Story into it
- **Indentation**: Maintain 2-space Story indent (from Epic), 4-space Task indent (from Epic)
- **Preserve Structure**: Don't reformat existing content

---

**Version:** 7.0.0 (BREAKING: Epic grouping algorithm for Story movement - preserves Epic context across status sections with split/merge logic)
