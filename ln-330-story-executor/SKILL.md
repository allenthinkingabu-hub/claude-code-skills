...
- **Phase 3 Loop (priority):**
  1) To Review -> ln-332-task-reviewer (one task). Reload metadata after worker.
  2) To Rework -> ln-333-task-rework (one task). After worker, verify status = To Review.
  3) Todo -> choose first Todo; if label "tests" use ln-334-test-executor else ln-331-task-executor. After worker, verify status = To Review (not Done/In Progress). Reload metadata and repeat until no To Review/To Rework/Todo.
...
- Status discipline: after ln-331/ln-333/ln-334, task must be To Review. Only ln-332 may set Done. Stop and report if any worker leaves task Done or In Progress.
...
- Loop executed: all To Review via ln-332; all To Rework via ln-333 (validated To Review after); all Todo via ln-331/ln-334 (validated To Review after).
